...
def margin(caxis=1,weights=1.0,mpos=0.9,mneg=0.1,alpha=0.5,reduction='mean'):
    def _margin_loss(preds, trues):
        ...
        loss = t.float() * F.relu(mpos-preds).pow(2) + alpha * (1.0-t.float()) * F.relu(preds-mneg).pow(2)
        loss = loss * weights
...
def smoothl1(*args, **kwargs):
    loss = nn.SmoothL1Loss(*args, **kwargs)
    def _smoothl1(preds, trues):
        return {'sl1':loss(preds, trues)}
    return _smoothl1
...
__losses__ = { 'l1': l1,
               ...
               'xlsoftmargin': xlsoftmargin,
               'cosine': cosine,
               'xmargin': xmargin,
               'tripletmargin': tripletmargin}


def get(loss):
    if loss is None or isinstance(loss, _Loss) or callable(loss):
        return loss
