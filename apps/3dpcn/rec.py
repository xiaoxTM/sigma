    ...
    with sess:
        ...
        for epoch in range(args.epochs):
            ...
            for iters in range(train_iters):
                ...
                trainc = helpers.one_hot(train_oid, args.nclass)
                _, loss, _, summary = ops.core.run(sess, [train_op, train_loss_op, train_metric_update_op, summarize], {inputs:trainx, labels:trainc})
            ...
            for iters in range(valid_iters):
                ...
                validc = helpers.one_hot(valid_oid, args.nclass)
                loss, _ = ops.core.run(sess, [valid_loss_op, valid_metric_update_op], {inputs:validx, labels:validc})
            ...
            for iters in range(test_iters):
                ...
                testc = helpers.one_hot(test_oid, args.nclass)
                loss, _ = ops.core.run(sess, [valid_loss_op, valid_metric_update_op], {inputs:testx, labels:testc})
                accuracy = ops.core.run(sess, valid_metric_op)
