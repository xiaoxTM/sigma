...
def build_net(inputs, views=32, nclass=40, reuse=False, num_gpus=4):
    ...
    for view in range(views):
        with ops.core.device('/gpu:{}'.format(view%num_gpus)):
            x = order_invariance_transform(x, 9, reuse=reuse, name='projection_0-{}'.format(view), act='squash')
            if not reuse:
                ops.core.summarize('projection_0_{}'.format(view), x)
            x = order_invariance_transform(x, 16, reuse=reuse, name='projection_1-{}'.format(view), act='squash')
            if not reuse:
                ops.core.summarize('projection_1_{}'.format(view), x)
            x = order_invariance_transform(x, 32, reuse=reuse, name='projection_2-{}'.format(view), act='squash')
            if not reuse:
                ops.core.summarize('projection_2_{}'.format(view), x)
            x = layers.capsules.dense(x, 32, 32, epsilon=1e-10, reuse=reuse, name='caps_fully_connected-{}'.format(view), act='squash',
                    share_weight=True)
            if not reuse:
                ops.core.summarize('caps_fully_connected-{}'.format(view), x)
            multiviews.append(x)
    with ops.core.device('/gpu:0'):
        x = layers.merge.concat(multiviews, axis=1, reuse=reuse, name='concatenate')
        if not reuse:
            ops.core.summarize('concatenate', x)
        x = layers.capsules.dense(x, nclass, 16, epsilon=1e-10, reuse=reuse, name='caps_fully_connected', act='squash')
        if not reuse:
            ops.core.summarize('cap_fully_connected', x)
        x = layers.capsules.norm(x, axis=1, safe=True, epsilon=1e-10, name='norm')
        if not reuse:
...
def train_net(batch_size=8,
    ...
    if database == 'shapenet_part':
        ...
        from dataset.modelnet40 import parse
        train_filename = '/home/xiaox/studio/db/modelnet/train2048.tfrecord'
        valid_filename = None
        tests_filename = '/home/xiaox/studio/db/modelnet/test2048.tfrecord'
