...
def per_channel_routing(inputs,
    ...
    output_shape = [batch_size, indims, nroutings] # for concatenating along dims
    logits_shape = [batch_size, 1, 1, incaps]
    bias_shape = [1, nroutings]
    bias = ops.mm.malloc('bias',
    ...
    def _routing(idx, x, preds):
        ...
        subx = ops.core.reshape(subx, (batch_size, indims, 1, incaps))
        #=> subx  : [batch-size, dims, 1]
    ...
    def _per_channel_routing(x):
        ...
        # [nroutings, batch-size, dims, 1]
        predictions = predictions.stack()
        # [batch-size, dims*nroutings, 1]
        predictions = ops.core.transpose(predictions, (1, 2 ,0, 3))
        predictions = ops.core.reshape(predictions, (batch_size, indims, nroutings))
        return predictions
...
def build_net(inputs, nclass=40, reuse=False):
    ...
    x = per_channel_routing(x, 3, reuse=reuse, name='pcr', act='squash')
    if not reuse:
