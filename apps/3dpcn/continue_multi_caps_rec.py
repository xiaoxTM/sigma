...
parser.add_argument('--learning-rate', default=0.35, type=float)
...
@helpers.stampit({'checkpoint':-2, 'log':-1}, message='multi_caps_rec_shapenet_part')
def train_net(batch_size=8,
    ...
    base_filename = '/home/xiaox/studio/db/shapenet/shapenet_part/shapenetcore_partanno_segmentation_normalized'
    train_filename = '{}.tfrecord'.format(base_filename)
    valid_filename = '{}_valid.tfrecord'.format(base_filename)
    tests_filename = '{}_test.tfrecord'.format(base_filename)
    ...
    sess, saver, summarize, writer = engine.session(checkpoint='/home/xiaox/studio/exp/3dpcn/cache/20190816071745_multi_caps_rec_shapenet_part/checkpoint/model.ckpt',#checkpoint,
                                                    config=config,
                                                    debug=debug,
                                                    address=address,
                                                    log=log)
    ...
    with sess:
        ...
        for epoch in range(epochs):
            ...
            tloss = np.mean(valid_loss)
            tacc = np.mean(valid_acc)
            losses[epoch][2] = tloss
            losses[epoch][3] = tacc
            print('valid for {}-th epoch: loss:{}, accuracy: {}'.format(epoch, tloss, tacc))
            # test
            test_loss = []
            test_acc = []
        ...
        np.savetxt('multi_caps_rec_shapenet_part.log', losses)
