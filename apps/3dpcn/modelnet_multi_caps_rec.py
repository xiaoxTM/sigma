...
@helpers.stampit({'checkpoint':-2, 'log':-1}, message='modelnet_multi_caps_rec')
def train_net(batch_size=8,
    ...
    base_filename = '/home/xiaox/studio/db/modelnet'
    train_filename = os.path.join(base_filename, 'train2048.tfrecord')
    tests_filename = os.path.join(base_filename, 'test2048.tfrecord')
    ...
    with ops.core.device('/gpu:0'):
        trainp = build_net(inputs, nclass=nclass)
        ...
        validp = build_net(inputs, nclass=nclass, reuse=True)
