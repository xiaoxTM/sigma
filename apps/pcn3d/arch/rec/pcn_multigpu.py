...
def build_net(inputs, labels, loss='cce', batch_size=32, nclass=16, reuse=False, is_training=True, **kwargs):
    ...
    pcts = []
    for i in range(12):
        with core.device('/gpu:{}'.format(i%3)):
            # [batch-size, npoints, 1, 64]
    ...
    with core.device('/gpu:2'):
        ...
        pct = layers.capsules.dense(pct, 256, 8, iterations=3, name='cap_dense-0', reuse=reuse)
        pct = layers.norms.batch_norm(pct, is_training, momentum=0.9, name='batch_norm-0', reuse=reuse)

        pct = layers.capsules.dense(pct, 128, 8, iterations=3, name='cap_dense-1', reuse=reuse)
        pct = layers.norms.batch_norm(pct, is_training, momentum=0.9, name='batch_norm-1', reuse=reuse)

        pct = layers.capsules.dense(pct, 40, 16, iterations=3, name='cap_dense-2', reuse=reuse)
