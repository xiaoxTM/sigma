...
def build_net(inputs, views=2, nclass=40, reuse=False, num_gpus=4):
    ...
    #=>      [batch-size, 3, 2048]
    begin = layers.base.transpose(inputs, (0, 2, 1))
    #        [batch-size, 3, 2048]
    #=>      [batch-size, 6,  512]
    if not reuse:
        ops.core.summarize('inputs', begin)
    gpus = range(num_gpus)
    multiviews = []
    for view in range(views):
        with ops.core.device('/gpu:{}'.format(view%num_gpus)):
            x = permutation_transform(begin, 9, reuse=reuse, name='projection_0-{}'.format(view), act='squash')
            ops.core.summarize('projection_0_{}'.format(view), x, reuse=reuse)
            ...
            x = layers.capsules.dense(x, 16, 9, epsilon=1e-10, reuse=reuse, name='caps_fully_connected-{}'.format(view), act='squash',
                    share_weights=True)
            ops.core.summarize('caps_fully_connected-{}'.format(view), x, reuse=reuse)
            multiviews.append(x)
    with ops.core.device('/gpu:0'):
        x = layers.merge.concat(multiviews, axis=1, reuse=reuse, name='concatenate')
        ops.core.summarize('concatenate', x, reuse=reuse)
        x = layers.capsules.dense(x, nclass, 16, epsilon=1e-10, reuse=reuse, name='caps_fully_connected', act='squash')
        ops.core.summarize('caps_fully_connected', x, reuse=reuse)
        x = layers.capsules.norm(x, axis=1, safe=True, epsilon=1e-10, name='norm', reuse=reuse)
        ops.core.summarize('norm', x, reuse=reuse)
    return x
...
def train_net(batch_size=8,
    ...
    if database == 'shapenet_part':
        ...
        from dataset.modelnet40_loader import parse
    ...
    trainp = build_net(inputs, num_gpus=num_gpus)
    validp = build_net(inputs, reuse=True, num_gpus=num_gpus)
    with ops.core.device('/gpu:0'):
    ...
    with sess:
        ...
        print('trainable parameters:', ops.core.trainable_parameters())
