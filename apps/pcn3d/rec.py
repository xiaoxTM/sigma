...
def train_net(batch_size=8,
    ...
    train_pred, train_loss_op, train_metric = rec.build_net(net_arch, inputs, labels,
                                                            batch_size=batch_size,
                                                            nclass=nclass,
                                                            is_training=True,
                                                            reuse=False,
                                                            views=views, num_gpus=num_gpus)
    valid_pred, valid_loss_op, valid_metric = rec.build_net(net_arch, inputs, labels,
                                                            batch_size=batch_size,
                                                            nclass=nclass,
                                                            reuse=True,
                                                            is_training=False,
    ...
    with sess:
        ...
        for epoch in range(epochs):
            ...
            for iters in range(train_iters):
                pred, true, _, loss, _, summary = ops.core.run(sess, [train_pred, labels, train_op, train_loss_op, train_metric_update_op, summarize])
